...
# read wings manifest if exists
$wingsManifestPath = Join-Path $wingsRoot 'manifest.json'
$wings = @()
if(Test-Path $wingsManifestPath){
  try { $w = Get-Content -LiteralPath $wingsManifestPath -Raw | ConvertFrom-Json; if($w){ $wings = $w } } catch {}
}else{
  # fallback: probe textures like wingNNNN_c.png → group by numeric
  $texDir = Join-Path $wingsRoot 'textures'
  if(Test-Path $texDir){
    $pngs = Get-ChildItem -LiteralPath $texDir -File -Filter *.png -ErrorAction SilentlyContinue
    $groups = @{}
    foreach($p in $pngs){
      if($p.BaseName -match '(?i)wing(\d+)'){ $n=$Matches[1]; if(!$groups.ContainsKey($n)){ $groups[$n]=@() }; $groups[$n]+=$p.Name }
    }
...
if($World){
  ...
  $scene = [ordered]@{
    world = $World
    selections = @($combos | Select-Object -First ([Math]::Min($MaxCombos, $combos.Count)))
    camera = @{ position = @(0,1.4,2.5); target = @(0,1.3,0) }
  }
