param(
  [string]$Root=".",
  [switch]$ArchiveOrphans,
  [string]$ArchiveDir="scripts/_archive"
)
$ErrorActionPreference="Stop"
$here = Split-Path -Parent $PSScriptRoot
$repo = Join-Path $here $Root

# Load refs and inventory
...
# Calculate "used" for scripts from workflows + scripts call graph
...
# closure over script->script edges
...
$out = @{
  ...
  totals=@{ files=$inv.count; scripts=$allScripts.Count; used=$used.Count; orphans=$orphans.Count }
  used=[array]$used
  orphans=$orphans
}
...
if($ArchiveOrphans){
  ...
  foreach($p in $orphans){
    ...
    if(Test-Path $src){
      $dest = Join-Path $batch (Split-Path -Leaf $src)
