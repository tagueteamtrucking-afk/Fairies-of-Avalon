...
$patterns = @(
  ...
  "timeline-$World.json",
  "npcs-$World.json",
  "codex/figures-$World.json",
  "reports/continuity-$World.json",
  "exports/Scenes-$World.md"
)
$files = @()
foreach($pat in $patterns){
  $p = Join-Path $worldsDir $pat
  $found = Get-ChildItem -LiteralPath $p -ErrorAction SilentlyContinue
  if($found){ $files += $found.FullName }
}

$zipPath = Join-Path $exportDir ("WorldBundle-" + $World + ".zip")
if(Test-Path $zipPath){ Remove-Item -LiteralPath $zipPath -Force }
$zip = [System.IO.Compression.ZipFile]::Open($zipPath, [System.IO.Compression.ZipArchiveMode]::Create)
foreach($f in $files){
  $rel = $f.Replace($worldsDir, "").TrimStart('\','/')
