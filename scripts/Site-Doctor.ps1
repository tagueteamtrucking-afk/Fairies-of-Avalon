...
param([string]$Domain = "", [bool]$ApplyFixes = $true)
...
if ($ApplyFixes) {
  ...
  if (-not $hasKill) {
    $html = @"
<!doctype html><meta charset="utf-8"><title>Kill SW</title>
<body style="font-family:system-ui;padding:24px"><h1>Kill Service Worker</h1><pre id="log"></pre>
...
log("Done. Close this tab and reload the site.");})();</script></body>
"@
...
$report = @{ updated=$now; domain=$Domain; planned=$planned; applied=$ApplyFixes }
