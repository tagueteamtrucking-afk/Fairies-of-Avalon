# Optional: Azure TTS — uses AZURE_TTS_KEY and AZURE_TTS_REGION to synthesize audio.
param(
...
if ([string]::IsNullOrWhiteSpace($region) -or [string]::IsNullOrWhiteSpace($key)) {
  Write-Host "Azure TTS secrets not set. Skipping."
  exit 0
}
...
"@
  ...
  try {
    ...
    Write-Warning "TTS failed for $($p.id): $_"
  }
}
