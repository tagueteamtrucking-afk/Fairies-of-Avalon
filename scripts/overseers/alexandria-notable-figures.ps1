...
function Make-Name(){
  $a = Pick $CH.name_syllables_a
  $b = Pick $CH.name_syllables_b
  $c = Pick $CH.name_syllables_a
  $n = ($a + $b + $c)
  return ($n.Substring(0,1).ToUpper() + $n.Substring(1))
}

# settlements come from atlas if present; else synthesize a few towns
$atlas = $null
$atlasPath = Join-Path $worldsDir ("atlas-" + $World + ".json")
...
if($atlas -and $atlas.regions){
  foreach($r in $atlas.regions){
    if($r.settlements){ $settlements += @($r.settlements | ForEach-Object { $_.name }) }
  }
}
if($settlements.Count -eq 0){
  # synthesize 3 towns
  for($i=0;$i -lt 3;$i++){
    $settlements += ("Town " + (Make-Name()))
  }
}
...
foreach($town in $settlements){
  $list = @()
  for($i=0;$i -lt [Math]::Max(1,$PerTown);$i++){
    $list += [ordered]@{
      name      = Make-Name()
      role      = Pick $CH.professions
...
$outDir = Join-Path $worldsDir 'codex'
